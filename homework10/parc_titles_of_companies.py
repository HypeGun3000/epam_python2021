...
def convert_dollars_in_rub(company):
...
def multiprocessing_convert():
    ...
    with Pool(50) as pool:
        company_page_info = pool.map(convert_dollars_in_rub, [company for company in company_page_info])
    return company_page_info
...
if __name__ == "__main__":
    ...
    top_10_lowest_p_e = sorted(company_page_info, key=lambda rubl: rubl[2], reverse=False)[:10]
